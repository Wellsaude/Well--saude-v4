...
export default function Assinar(){
  ...
  useEffect(()=>{const r=new URLSearchParams(window.location.search).get('ref'); if(r) setRef(r)},[])
  async function pagar(){
    setLoading(true)
    const res=await fetch('/api/checkout',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({...f,afiliado_de:ref,forma_pagamento:forma})})
    const d=await res.json()
    if(d.invoiceUrl||d.url) location.href=d.invoiceUrl||d.url
    else{ alert(JSON.stringify(d)); setLoading(false) }
  }
  ...
  return(
    <div style={{padding:20,maxWidth:420,margin:'auto',fontFamily:'sans-serif'}}>
      ...
      <div style={{display:'flex',gap:8,marginTop:12}}>
        ...
        <button onClick={()=>setForma('CREDIT_CARD')} style={{flex:1,padding:10,background:forma==='CREDIT_CARD'?'#0070f3':'#999',color:'white',border:0,borderRadius:8}}>CARTÃO</button>
      </div>
      ...
      <p style={{fontSize:11,marginTop:10}}>Todo cliente vira afiliado: ?ref=SEU_CPF • 8,33 pts indicação + 8,33 pts adimplência = catálogo + cartão Well futuro</p>
    </div>
  )
    }
