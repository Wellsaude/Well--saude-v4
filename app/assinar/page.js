// >>> PAGINA ASSINAR - FORMULARIO COM PIX <<<
"use client"
...
<button onClick={()=>setForma('PIX')} style={{padding:10,background:forma==='PIX'?'green':'gray',color:'#fff',marginRight:8}}>PIX</button>
<button onClick={()=>setForma('CREDIT_CARD')} style={{padding:10,background:forma==='CREDIT_CARD'?'blue':'gray',color:'#fff'}}>CARTAO</button>
<button onClick={pagar} style={{width:'100%',padding:16,background:'#00a859',color:'#fff',marginTop:16}}>ASSINAR {forma}</button>
</div>
)
}
