...
export async function POST(req){
  ...
  const customerRes = await fetch('https://api.asaas.com/v3/customers',{
    ...
    body: JSON.stringify({ name: nome, cpfCnpj: cpf, email, mobilePhone: whatsapp })
  })
  ...
  const subRes = await fetch('https://api.asaas.com/v3/subscriptions',{
    ...
    body: JSON.stringify({
      ...
      billingType: forma_pagamento, // PIX ou CREDIT_CARD
      value: 43.5,
  ...
  return NextResponse.json({ invoiceUrl: sub.subscription?.invoiceUrl || sub.invoiceUrl, sub })
      }
